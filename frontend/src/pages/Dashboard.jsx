...
const shouldShowInVulnerable = (dep) =>
  (dep.risk || "").toUpperCase() !== "LOW";
...
const computeRiskTone = (level, score) => {
  ...
  if (lv === "HIGH")
    return {
      from: "#ef4444",
      to: "#ff761f",
      glow: "bg-severity-high",
      text: "text-severity-high",
    };
  if (lv === "MEDIUM" || lv === "MODERATE")
    return {
      from: "#eab308",
      to: "#f59e0b",
      glow: "bg-severity-moderate",
      text: "text-severity-moderate",
    };
  return {
    from: "#22c55e",
    to: "#86efac",
    glow: "bg-severity-low",
...
function ScoreRing({ label, value, tone, badge, description }) {
  ...
  return (
    <div className="bento relative flex flex-col items-center overflow-hidden px-6 pb-6 pt-8">
      <div
        className={`pointer-events-none absolute -top-10 h-36 w-36 rounded-full blur-3xl opacity-25 ${tone.glow}`}
      />
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="flex items-baseline gap-1">
            <span
              className={`text-[46px] font-bold leading-none tracking-tight ${tone.text}`}
            >
              {display}
            </span>
          ...
          {badge && (
            <span
              className="mt-2.5 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[.12em]"
              style={{
                borderColor: `${tone.from}66`,
                backgroundColor: `${tone.from}1a`,
                color: tone.from,
              }}
            >
              {badge}
...
export default function Dashboard({ githubToken }) {
  ...
  const [repoUrl, setRepoUrl] = useState(
    () => localStorage.getItem("cached_repo_url") || "",
  );
  const [localPath, setLocalPath] = useState(
    () => localStorage.getItem("cached_local_path") || "",
  );
  ...
  const handleFolderChange = async (e) => {
    ...
    try {
      ...
      setScanResult({
        error: `Failed to read manifest files in "${folderName}": ${readErr.message}`,
      });
      return;
    }
    ...
    setScanProgress(15);
    setScanStage(
      `Found ${manifests.length} dependency manifest(s) in "${folderName}". Initiating scan...`,
    );
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let msg =
          errorData.detail || "Failed to initiate scan from manifest files.";
        if (typeof msg !== "string")
          msg = Array.isArray(msg) ? msg[0]?.msg : JSON.stringify(msg);
        throw new Error(msg);
      }
  ...
  const handleScan = async (url = repoUrl) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "Invalid GitHub repository URL format. Please use https://github.com/owner/repo",
        );
      }
      ...
      try {
        ...
        if (ghRes.ok) {
          ...
          if (!repoData.permissions?.admin && !repoData.permissions?.push) {
            throw new Error(
              "Security Error: You do not have write or admin access to this repository. You can only scan repositories you own.",
            );
          }
        } else if (ghRes.status === 404 || ghRes.status === 403) {
          throw new Error(
            'Security Error: You do not have access to this repository. If this is a private repo, ensure your Personal Access Token has the "repo" scope checked.',
          );
        }
        ...
        console.warn(
          "GitHub API pre-check skipped (network unavailable):",
          ghErr.message,
        );
      }
  ...
  const handleLocalScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let msg =
          errorData.detail ||
          "Failed to initiate local scan. Make sure the path is correct and contains a supported dependency manifest (package.json, requirements.txt, go.mod, pyproject.toml, etc.).";
        if (typeof msg !== "string")
          msg = Array.isArray(msg) ? msg[0]?.msg : JSON.stringify(msg);
        throw new Error(msg);
      }
  ...
  const openRepoDropdown = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          res.status === 401
            ? "GitHub token invalid or expired. Please sign in again."
  ...
  useEffect(() => {
    ...
    const onDocClick = (e) => {
      if (
        ...
        !repoDropdownRef.current.contains(e.target)
      ) {
        setIsRepoDropdownOpen(false);
      }
    };
      ...
      try {
        ...
        if (data.status === "COMPLETED") {
          ...
          setScanResult({
            error:
              data.error_message ||
              "Repository scanning failed. Check backend logs.",
          });
          setIsScanning(false);
          setPollingId(null);
          setScanProgress(0);
          ...
          stalls += 1;
          if (stalls > 150) {
            // ~5 minutes with no status change
            setScanResult({
              error:
                "The scan is taking too long. Please check that the backend is running and try again.",
            });
            setIsScanning(false);
            setPollingId(null);
            setScanProgress(0);
        ...
        if (consecutiveFailures >= 6) {
          // ~12s of unreachable backend
          setScanResult({
            error:
              "Lost connection to the scan backend. Please make sure the backend is running and try again.",
          });
          setIsScanning(false);
          setPollingId(null);
          setScanProgress(0);
  ...
  const processedDeps = useMemo(() => {
    ...
    let list = showOnlyVulnerable
      ? scanResult.dependencies.filter(shouldShowInVulnerable)
    ...
    if (vulnSeverity !== "ALL")
      list = list.filter((d) => (d.risk || "").toUpperCase() === vulnSeverity);
    if (vulnTopology !== "ALL")
      list = list.filter((d) =>
        vulnTopology === "DIRECT" ? !!d.direct : !d.direct,
      );
    ...
    switch (vulnSort) {
      ...
      case "name-desc":
        ...
        break;
      case "severity-asc":
        list.sort((a, b) => sevRank(a) - sevRank(b));
        break;
      case "topology-direct":
        list.sort((a, b) => (b.direct ? 1 : 0) - (a.direct ? 1 : 0));
        break;
      case "topology-transitive":
        list.sort((a, b) => (a.direct ? 1 : 0) - (b.direct ? 1 : 0));
        break;
      case "severity-desc":
      default:
        list.sort((a, b) => sevRank(b) - sevRank(a));
        break;
    }
    return list;
  }, [
      ...
      <section className="mx-auto w-full max-w-[1500px] pt-8 md:pt-12">
        ...
        >
          ...
          >
            <div className="flex h-[46px] items-center gap-2 rounded-[22px] bg-[#1d1d1d] px-3 ring-1 ring-white/[0.12] shadow-[inset_0_0_50px_rgba(255,255,255,.02)]">
              ...
              <input
                ...
                className="min-w-0 flex-1 bg-transparent text-[15px] text-white outline-none placeholder:text-[#8b8b8b]"
                onKeyDown={(e) => {
                  if (e.key === "Enter" && repoUrl) handleScan();
                  if (e.key === "Escape") setIsRepoDropdownOpen(false);
                }}
              />
              <div>
                <button
                  ...
                  className="grid h-8 w-8 place-items-center rounded-lg text-muted transition-colors hover:bg-white/[.05] hover:text-primary"
                >
                  <CaretDown
                    size={18}
                    className={isRepoDropdownOpen ? "rotate-180" : ""}
                  />
                </button>
              </div>
              <div ref={localMenuRef} className="relative">
                <button
                  ...
                  disabled={isScanning}
                  title="Select a local folder or zip"
                  className="grid h-9 w-9 place-items-center rounded-lg text-muted hover:bg-white/[.05] hover:text-primary disabled:opacity-40"
                >
                  <Folder size={19} />
                </button>
                {isLocalMenuOpen && (
                  <div className="absolute right-0 top-[calc(100%+10px)] z-[9999] w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#1c1815] shadow-2xl">
                    <div className="border-b border-white/[.07] px-4 py-3 text-xs font-semibold text-on-dark">
                      Import local project
                    </div>
                    <button
                      type="button"
                      onClick={handleChooseFolder}
                      className="flex w-full items-center gap-3 border-b border-white/[.05] px-4 py-3 text-left hover:bg-white/[.04]"
                    >
                      <Folder size={16} className="text-primary" />
                      <span className="min-w-0 flex-1 truncate text-sm text-on-dark">
                        Upload folder
                      </span>
                    </button>
                    <button
                      ...
                      className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-white/[.04]"
                    >
          ...
          <ActionSearchBar
            ...
            error={reposError}
            onSelect={(action) =>
              selectRepo({ id: action.id, full_name: action.label })
            }
        ...
        {!scanResult && !isScanning && (
          <div className="mx-auto mt-12 max-w-3xl">
            <h2 className="mb-3 text-sm font-semibold text-on-dark">History</h2>
            <div className="max-h-[calc(100vh-270px)] overflow-y-auto rounded-[18px] border border-white/[.08] bg-surface-card-dark/60 p-1">
              {history.length ? (
                history.map((scan) => (
                  <button
                    key={scan.id}
                    onClick={() =>
                      scan.status === "COMPLETED" && setPollingId(scan.id)
                    }
                    className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[13px] px-4 py-3 text-left hover:bg-white/[.04]"
                  >
                    <span
                      className={`h-2.5 w-2.5 rounded-full ${scan.status === "COMPLETED" ? "bg-trading-up" : scan.status === "FAILED" ? "bg-trading-down" : "bg-primary"}`}
                    />
                    <span className="min-w-0">
                      <span className="block truncate text-sm text-on-dark">
                        {(scan.repository_url || "Local analysis").replace(
                          /^(zip|local|folder):\/\//,
                          "",
                        )}
                      </span>
                ...
                <div className="flex min-h-40 items-center justify-center rounded-[14px] border border-dashed border-white/[.08] text-sm text-muted">
                  Previously analyzed repositories and local projects will
                  appear here.
                </div>
              )}
            </div>
          ...
          {isScanning && !scanResult ? (
            <div className="bg-surface-card-dark border border-hairline-on-dark rounded-2xl p-8 md:p-10 max-w-2xl mx-auto shadow-2xl relative overflow-hidden backdrop-blur-sm">
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <h3 className="text-title-lg text-on-dark font-semibold tracking-tight mb-2">
                ...
                <div className="w-full bg-surface-elevated-dark rounded-full h-3.5 p-0.5 border border-hairline-on-dark relative overflow-hidden mb-3">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round(scanProgress))}%`,
                    }}
                  >
                    {/* Animated shine line */}
                ...
                <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-hairline-on-dark/60 text-left">
                  <div
                    className={`text-caption p-2 rounded-lg border transition-all ${scanProgress >= 25 ? "bg-primary/10 border-primary/40 text-primary" : "bg-surface-dark border-hairline-on-dark/40 text-muted opacity-60"}`}
                  >
                    <div className="font-semibold flex items-center gap-1.5">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${scanProgress >= 25 ? "bg-primary" : "bg-muted"}`}
                      ></span>
                      Clone Repo
                    </div>
                  </div>
                  <div
                    className={`text-caption p-2 rounded-lg border transition-all ${scanProgress >= 50 ? "bg-primary/10 border-primary/40 text-primary" : "bg-surface-dark border-hairline-on-dark/40 text-muted opacity-60"}`}
                  >
                    <div className="font-semibold flex items-center gap-1.5">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${scanProgress >= 50 ? "bg-primary" : "bg-muted"}`}
                      ></span>
                      Parse Tree
                    </div>
                  </div>
                  <div
                    className={`text-caption p-2 rounded-lg border transition-all ${scanProgress >= 75 ? "bg-primary/10 border-primary/40 text-primary" : "bg-surface-dark border-hairline-on-dark/40 text-muted opacity-60"}`}
                  >
                    <div className="font-semibold flex items-center gap-1.5">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${scanProgress >= 75 ? "bg-primary" : "bg-muted"}`}
                      ></span>
                      Query OSV
                    </div>
                  </div>
                  <div
                    className={`text-caption p-2 rounded-lg border transition-all ${scanProgress >= 95 ? "bg-primary/10 border-primary/40 text-primary" : "bg-surface-dark border-hairline-on-dark/40 text-muted opacity-60"}`}
                  >
                    <div className="font-semibold flex items-center gap-1.5">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${scanProgress >= 95 ? "bg-primary" : "bg-muted"}`}
                      ></span>
                      Risk Scoring
                    </div>
                  </div>
            ...
            <div className="bg-surface-card-dark border border-trading-down rounded-xl p-8 text-center max-w-2xl mx-auto">
              ...
              <h3 className="text-title-lg text-trading-down mb-2">
                Analysis Failed
              </h3>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <ScoreRing
                  label="Context risk"
                  value={scanResult.score ?? 0}
                  tone={computeRiskTone(scanResult.level, scanResult.score)}
                  badge={scanResult.level || "Assessing"}
                  description="Threat level across the dependency graph"
                />
                <ScoreRing
                  label="Project health"
                  value={Math.max(0, 100 - (scanResult.score ?? 0))}
                  tone={{
                    from: "#0ecb81",
                    to: "#2dbdb6",
                    glow: "bg-trading-up",
                  ...
                  description="Dependency posture & maintenance score"
                />
                <ScoreRing
                  label="Signal confidence"
                  value={scanResult.confidence ?? 86}
                  tone={{
                    from: "#3b82f6",
                    to: "#38bdf8",
                    glow: "bg-info",
                  ...
                  description="Evidence quality behind every finding"
                />
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="bento lg:col-span-4 p-5">
                  <div className="mb-5 flex items-center justify-between">
                    <div>
                      <p className="eyebrow mb-1">Project structure</p>
                      <h3 className="text-sm font-semibold text-on-dark">
                        {getProjectLabel(repoUrl, localPath)}
                      </h3>
                    </div>
                    <Folder size={18} className="text-primary" />
                  </div>
                  <div className="space-y-1 text-sm">
                    <div className="rounded-lg px-3 py-2 text-body">
                      ⌄ &nbsp; root
                    </div>
                    <div className="rounded-lg px-3 py-2 pl-8 text-muted">
                      ├─ package manifests
                    </div>
                    <div className="rounded-lg px-3 py-2 pl-8 text-muted">
                      ├─ dependencies
                  ...
                  <button
                    onClick={() => setShowOnlyVulnerable((v) => !v)}
                    className={`mt-5 w-full rounded-lg border py-2.5 text-xs transition-colors ${showOnlyVulnerable ? "border-primary bg-primary/15 text-primary" : "border-white/[.08] text-body hover:border-primary/40 hover:text-primary"}`}
                  >
                    {showOnlyVulnerable
                      ? "Showing vulnerable files"
                      : "Show vulnerable files only"}
                  </button>
                </div>
                ...
                <div className="bento lg:col-span-8 p-5">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-title-md text-on-dark">
                    ...
                    <div className="flex gap-4 border-b border-hairline-on-dark">
                      <button className="text-body-sm font-medium text-primary border-b-2 border-primary pb-2 px-1">
                        ...
                        (
                        {showOnlyVulnerable
                          ? (scanResult.dependencies || []).filter(
                              shouldShowInVulnerable,
                            ).length
                  ...
                  <div className="mb-4 space-y-3">
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="relative min-w-[220px] flex-1">
                        ...
                        {vulnSearch && (
                          <button
                            ...
                            title="Clear search"
                            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted transition-colors hover:text-on-dark"
                          >
                            <X size={14} />
                          </button>
                        )}
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <select
                          ...
                          className="cursor-pointer rounded-lg border border-white/[.08] bg-surface-elevated-dark px-3 py-2 text-xs text-on-dark outline-none transition-colors focus:border-primary/60"
                        >
                        ...
                        <label className="sr-only" htmlFor="topo-filter">
                          Topology
                        </label>
                        <select
                          id="topo-filter"
                          value={vulnTopology}
                          onChange={(e) => setVulnTopology(e.target.value)}
                          className="cursor-pointer rounded-lg border border-white/[.08] bg-surface-elevated-dark px-3 py-2 text-xs text-on-dark outline-none transition-colors focus:border-primary/60"
                        >
                        ...
                        <select
                          id="sort-filter"
                          value={vulnSort}
                          onChange={(e) => setVulnSort(e.target.value)}
                          className="cursor-pointer rounded-lg border border-white/[.08] bg-surface-elevated-dark px-3 py-2 text-xs text-on-dark outline-none transition-colors focus:border-primary/60"
                        >
                          ...
                          <option value="topology-transitive">
                            Transitive first
                          </option>
                        </select>
                      </div>
                    ...
                    <div className="text-xs text-muted">
                      {processedDeps.length} of{" "}
                      {scanResult.dependencies?.length || 0} dependencies shown
                    </div>
                  </div>
                    ...
                    {processedDeps.length > 0 ? (
                      processedDeps.map((dep, i) => (
                        <div
                          key={i}
                          onClick={() =>
                            navigate(`/finding/${dep.finding_id || dep.id}`, {
                              state: {
                                finding: dep,
                        ...
                        >
                          <div className="col-span-5 flex items-center gap-3">
                            <div
                              className="h-6 px-2.5 rounded-full bg-surface-elevated-dark flex items-center justify-center font-plex text-xs text-primary truncate shrink-0 max-w-[90px]"
                              title={dep.ecosystem || "npm"}
                            >
                              {dep.ecosystem || "npm"}
                            </div>
                            <span
                              className="text-number-md text-on-dark truncate"
                              title={dep.id}
                            >
                              {dep.id}
                            </span>
                          ...
                          <div className="col-span-3 text-right font-plex text-number-md">
                            {dep.risk === "CRITICAL" ? (
                              ...
                              dep.is_reviewed || dep.decision ? (
                                <span className="text-trading-up font-medium">{dep.decision || "REVIEWED"}</span>
                              ) : (
                          ...
                          <div className="col-span-1 flex justify-end">
                            {(dep.risk || "").toUpperCase() === "UNKNOWN" ? (
                              dep.is_reviewed || dep.decision ? (
                                <span
                                  className="rounded-full bg-trading-up/20 border border-trading-up/30 text-trading-up px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider"
                                  title="Reviewed"
                                >
                                  {dep.decision || "Reviewed"}
                                </span>
                              ) : (
                                <span className="rounded-full bg-primary px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-ink">
                                  Review
                                </span>
                              )
                            ) : (
