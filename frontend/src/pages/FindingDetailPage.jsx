    ...
    <div className="flex flex-col md:flex-row items-center gap-6">
      <div className="relative w-48 h-48 shrink-0 flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-3xl font-bold font-plex text-on-dark">{score}</span>
          <span className="text-[10px] text-muted font-plex uppercase tracking-widest mt-0.5">RISK SCORE</span>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-2.5 w-full font-plex">
        {slices.map((s, i) => (
          <div key={i} className="flex items-center justify-between text-body-sm p-2 rounded-lg bg-surface-elevated-dark border border-hairline-on-dark/50">
...
function VulnerabilityHistoryGraph({ version = '1.0.0', vulnId = 'CVE-2021-23337', targetVersion = '1.1.0' }) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-title-sm text-on-dark flex items-center gap-2">
          <Clock size={16} className="text-primary" /> Vulnerability History & Lifecycle Map
        </h3>
        <span className="text-caption text-muted font-plex">Interactive Node Timeline</span>
      </div>
            ...
            {historyNodes.map((node, i) => (
              ...
              >
                ...
                <div className="text-center mt-3 font-plex">
                  <p className={`text-body-sm font-bold transition-colors ${activeNode === i ? 'text-primary' : 'text-on-dark'}`}>
                    {node.version}
                  </p>
                  <p className="text-caption text-muted mt-0.5 whitespace-nowrap">{node.status}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="p-4 bg-surface-card-dark border border-hairline-on-dark rounded-xl flex items-start gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 font-plex">
            <span className="text-body-sm font-bold text-on-dark">{active.version}</span>
            <span className="text-caption px-2 py-0.5 rounded-pill font-medium uppercase" style={{ backgroundColor: `${active.color}20`, color: active.color }}>
              {active.status}
            </span>
            <span className="text-caption text-muted">• {active.date}</span>
          </div>
          <p className="text-body-sm text-muted mt-1 leading-relaxed">{active.desc}</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      ...
      {needsReview && (
        <div className="p-4 bg-trading-down/10 border border-trading-down/30 rounded-xl flex items-center justify-between flex-wrap gap-3">
          ...
          <button 
            ...
            disabled={reviewing}
            className={`px-4 py-1.5 rounded-md text-caption font-button transition-colors flex items-center gap-1.5 ${
              isReviewed 
                ? 'bg-trading-up/20 text-trading-up hover:bg-trading-up/30' 
                : 'bg-primary hover:bg-primary-active text-ink'
            }`}
          >
            {isReviewed ? <><CheckCircle size={14}/> Reviewed</> : 'Mark Reviewed'}
          </button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-surface-card-dark rounded-xl border border-hairline-on-dark">
        <span className="text-body-sm text-muted">AI Assessment Confidence</span>
        <div className="flex items-center gap-2">
          <span className={`text-caption font-medium uppercase px-2 py-0.5 rounded-pill bg-surface-elevated-dark ${confidenceBadge(confidence?.level)}`}>
          ...
          <ul className="space-y-2">
            {evidenceList.map((e, i) => (
              <li key={i} className="flex items-start gap-2.5 text-body-sm text-body">
                <CheckCircle size={15} className="text-trading-up mt-0.5 shrink-0" />
        ...
        <CollapsibleSection title="Recommended Remediation" icon={Lightning}>
          <div className="space-y-3">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-body-md font-semibold text-on-dark capitalize">{recommendation.action}</span>
              {recommendation.target_version && (
                <span className="text-body-md font-plex text-primary font-bold">→ Upgrade to v{recommendation.target_version}</span>
              )}
            </div>
        ...
        <CollapsibleSection title="Verification Steps" icon={CheckCircle} defaultOpen={false}>
          <ol className="space-y-2">
            {verificationList.map((step, i) => (
              <li key={i} className="flex items-start gap-2.5 text-body-sm text-body">
                <span className="text-primary font-plex font-bold shrink-0 w-5">{i + 1}.</span>
            ...
            {sourcesList.map((s, i) => (
              <li key={i} className="flex items-start gap-2.5 bg-surface-elevated-dark p-3 rounded-lg border border-hairline-on-dark">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-body-sm font-medium text-on-dark truncate">{s.title || s.url}</p>
                  <a
                    href={s.url}
                    target="_blank"
                  ...
                  {s.authority && (
                    <span className="text-caption text-muted mt-1 inline-block">{s.authority}</span>
                  )}
                </div>
              </li>
            ))}
...
export default function FindingDetailPage() {
  ...
  const [dependencies, setDependencies] = useState(location.state?.dependencies || null)
  ...
  const runAnalysis = async (questionType = 'vulnerability', question = null) => {
    const idToUse = finding?.finding_id || (finding?.id && !isNaN(Number(finding.id)) ? Number(finding.id) : Number(findingId));
    ...
    try {
      ...
      setError(typeof e?.message === 'string' ? e.message : 'Failed to reach AI service.');
    } finally {
      setLoading(false);
    }
  }
  ...
  const [pkgName, pkgVersion] = (finding.id || `${finding.package_name}@${finding.version}`).split('@')
  const riskScoreVal = finding.risk_score != null ? Math.round(finding.risk_score) : 75
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1 text-caption text-muted flex-wrap font-plex">
                <span className={`font-semibold uppercase ${severityColor(severityVal)}`}>
                  {severityVal} SEVERITY
                </span>
                <span>•</span>
                <span>{finding.direct ? 'Direct Dependency' : 'Transitive Dependency'}</span>
                {finding.vulnerability_id && (
                  <>
                    <span>•</span>
                    <span className="text-accent-turquoise font-medium">{finding.vulnerability_id}</span>
                  </>
                )}
                <span>•</span>
